...
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=LGBMClassifier(boosting_type="dart", colsample_bytree=0.7, learning_rate=0.01, min_child_samples=500, n_estimators=20, n_jobs=1, num_leaves=250, subsample=0.9)),
    StackingEstimator(estimator=AdaBoostClassifier(algorithm="SAMME", learning_rate=0.5, n_estimators=100)),
    SelectFwe(score_func=f_classif, alpha=0.015),
    RandomForestClassifier(bootstrap=False, criterion="gini", max_features=0.1, min_samples_leaf=9, min_samples_split=18, n_estimators=100)
)
